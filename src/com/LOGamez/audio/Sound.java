...
public final class Sound implements LineListener {   
    ...
    /**EnShoot variable of Sound object*/
    ...
    /**P1Collect variable of Sound object*/
    ...
    */    
    public Sound(String filename, double vol){
        ...
        Clip oldSnd = SolarFox.sndMap.get(filename);
        if(oldSnd != null){
            this.aClip = oldSnd;
        } else {
            try{
                System.out.println("Loading Sound: "+SolarFox.soundCount+": " + audioFileName);
                ...
                setVolume(vol);
                SolarFox.sndMap.put(filename, aClip);
                SolarFox.soundCount++;
            } catch(Exception e){
            }
        }
    ...
    */    
    public Sound(String filename){
        ...
        AudioClip oldAudio = SolarFox.audioMap.get(filename);
        if(oldAudio != null){
            this.clip = oldAudio;
        } else {
            try{
                System.out.println("Loading Audio: "+SolarFox.audioCount+": "+"/snd"+ filename);
                clip = Applet.newAudioClip(Sound.class.getResource("/snd"+filename));
                SolarFox.audioMap.put(filename, clip);
                SolarFox.audioCount++;
            } catch (Exception e) {
            }
        }
    ...
    protected void finalize() throws Throwable {
        if(!fileName.isEmpty()){ 
            SolarFox.sndMap.remove(fileName);
            System.out.println("Removing Sound from memory: " + fileName);
        }
        if(!audioFileName.isEmpty()){ 
            SolarFox.audioMap.remove(audioFileName);
